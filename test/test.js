...
describe("contentDisposition.create(filename)", () => {
  it("should require a string", () => {
    assert.throws(contentDisposition.create.bind(null, 42), /filename.*string/);
  });

  it("should create a header with file name", () => {
    assert.strictEqual(
      contentDisposition.create("plans.pdf"),
  ...
  it("should use the basename of the string", () => {
    assert.strictEqual(
      contentDisposition.create("/path/to/plans.pdf"),
      'attachment; filename="plans.pdf"'
    );
  });
  ...
  describe('when "filename" is US-ASCII', () => {
    ...
    it("should escape quotes", () => {
      assert.strictEqual(
        contentDisposition.create('the "plans".pdf'),
  ...
  describe('when "filename" is ISO-8859-1', () => {
    ...
    it("should escape quotes", () => {
      assert.strictEqual(
        contentDisposition.create('the "plans" (1µ).pdf'),
  ...
  describe('when "filename" is Unicode', () => {
    it("should include filename* parameter", () => {
      assert.strictEqual(
        contentDisposition.create("планы.pdf"),
  ...
  describe('when "filename" contains hex escape', () => {
    it("should include filename* parameter", () => {
      assert.strictEqual(
        contentDisposition.create("the%20plans.pdf"),
    ...
    it("should handle Unicode", () => {
      assert.strictEqual(
        contentDisposition.create("€%20£.pdf"),
...
describe("contentDisposition.create(filename, options)", () => {
  ...
  describe('with "type" option', () => {
    ...
    it("should create a header with inline type", () => {
      assert.strictEqual(
        contentDisposition.create(undefined, { type: "inline" }),
        "inline"
      );
    });
    ...
    it("should create a header with inline type & filename", () => {
      assert.strictEqual(
        contentDisposition.create("plans.pdf", { type: "inline" }),
...
describe("contentDisposition.parse(string)", () => {
  it("should require string", () => {
    assert.throws(
      contentDisposition.parse.bind(null),
  ...
  it("should reject non-strings", () => {
    assert.throws(
      contentDisposition.parse.bind(null, 42),
      /argument string.*required/
    );
  });
  ...
  describe("with only type", () => {
    it("should reject quoted value", () => {
      assert.throws(
        contentDisposition.parse.bind(null, '"attachment"'),
        /invalid type format/
      );
    });
  ...
  describe("from TC 2231", () => {
    describe("Disposition-Type Inline", () => {
      ...
      it('should reject ""inline""', () => {
        assert.throws(
          contentDisposition.parse.bind(null, '"inline"'),
          /invalid type format/
        );
      });
